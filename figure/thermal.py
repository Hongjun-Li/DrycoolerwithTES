...
# 4. 绘图：仅展示 T_CDU_Sup 高于 45°C 的表现
...
ratio = ctrl_over_45_h / base_over_45_h if base_over_45_h > 1e-9 else np.nan

print(f"Controlled time >45°C: {ctrl_over_45_h:.2f} h")
print(f"Baseline time >45°C: {base_over_45_h:.2f} h")
print(f"Ratio (controlled/base): {ratio:.3f}")

fig, ax = plt.subplots(figsize=(12, 3.5))

ax.plot(res_ctrl['time'], t_ctrl_cdu_sup, color='green', linewidth=1.8)
ax.plot(res_base['time'], t_base_cdu_sup, color='gray', linestyle='--', linewidth=1.5)

# 高于 45°C 的区域填充，突出控制效果
ctrl_mask = t_ctrl_cdu_sup > 45.0
base_mask = t_base_cdu_sup > 45.0
ax.fill_between(res_ctrl['time'], 45.0, t_ctrl_cdu_sup, where=ctrl_mask, color='green', alpha=0.25)
...
legend_labels = [
    f'Controlled (>45°C: {ctrl_over_45_h:.2f} h)',
    f'Baseline (>45°C: {base_over_45_h:.2f} h)',
    '45°C Threshold',
    f'Ratio: {ratio:.2f}'
]
legend_lines = [
    plt.Line2D([0], [0], color='green', linewidth=1.8),
    plt.Line2D([0], [0], color='gray', linestyle='--', linewidth=1.5),
    plt.Line2D([0], [0], color='red', linestyle='--', linewidth=1.0),
