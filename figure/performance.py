...
# 4. 绘图：仅展示 PUE 与累计节约，共两列子图
saving_curve = np.array(res_base['cum_cost']) - np.array(res_ctrl['cum_cost'])
pue_ctrl_mean = np.mean(res_ctrl['pue'])
cost_saving_ratio = final_saving / res_base['cum_cost'][-1] if res_base['cum_cost'][-1] > 1e-9 else np.nan

print(f"Cost saving ratio (controlled/base): {cost_saving_ratio:.3f}")

fig, axes = plt.subplots(1, 2, figsize=(12, 3.5), sharex=True)
plt.subplots_adjust(wspace=0.25)

# 左：PUE
axes[0].plot(res_ctrl['time'], res_ctrl['pue'], color='darkred', linewidth=1.5, label=f'Controlled PUE (Mean: {pue_ctrl_mean:.3f})')
axes[0].set_ylim(0.9, 1.1)
axes[0].set_ylabel('PUE')
axes[0].set_title('Power Usage Effectiveness')
axes[0].legend(loc='upper center', fontsize='small')
...
axes[1].plot(
    ...
    linewidth=1.6,
    color='#f59e0b',
    label=f'Controlled Cost (Saving: {cost_saving_ratio:.2%})'
)
axes[1].set_ylabel('Cumulative Cost [USD]')
axes[1].set_xlabel('Time [Days]')
axes[1].set_title('Cumulative HVAC Electricity Cost')
