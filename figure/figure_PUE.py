...
try:
    ...
    raise SystemExit(
        ...
        "that includes pyfmi before running figure_PUE.py."
    ) from exc
...
FMU_OUTPUTS = ["ySOCtes", "yQflow", "yPHVAC", "yPIT", "yPUE"]

ROOT_DIR = Path(__file__).resolve().parent.parent
OUTPUT_PATH = Path(__file__).resolve().parent / "pue_hvac_comparison.png"
...
def run_city_simulation(city_config, use_control):
    ...
    for _ in tqdm(range(STEPS), desc=desc):
        ...
        fallback_pue = (c_pit + c_phvac) / c_pit if c_pit > 0 else 1.0
        ...
        data["pue"].append(y_pue if np.isfinite(y_pue) else fallback_pue)
...
def draw_figure(results):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(8.8, 2.25), sharex=True)
    fig.subplots_adjust(wspace=0.34, left=0.08, right=0.98, bottom=0.22, top=0.82)

    hvac_values = []
    for city_data in results.values():
        for scenario_data in city_data.values():
            hvac_values.extend(scenario_data["yPHVAC"])

    hvac_min = min(0.0, np.floor(min(hvac_values) / 100.0) * 100.0)
    hvac_max = np.ceil(max(hvac_values) / 100.0) * 100.0
    ...
    for col, city_name in enumerate(("New York", "Miami")):
        ...
        for scenario_key in ("tes", "no_tes"):
            ...
            line = ax_hvac.plot(
                scenario_data["time"],
                scenario_data["yPHVAC"],
                color=line_colors[scenario_key],
                linewidth=1.3,
                alpha=0.95,
                label=scenario_labels[scenario_key],
            )[0]
            hvac_handles.append(line)

        ax_hvac.set_title(f"{city_name}\nCooling Power Consumption")
        ax_hvac.set_ylabel("HVAC Power [W]")
        ax_hvac.set_ylim(hvac_min, hvac_max)
        ax_hvac.legend(hvac_handles, [h.get_label() for h in hvac_handles], loc="upper right", frameon=False)
...
def main():
    ...
    for city_name, city_results in results.items():
        base_cost = city_results["no_tes"]["cum_cost"][-1]
        tes_cost = city_results["tes"]["cum_cost"][-1]
        saving = base_cost - tes_cost
        saving_pct = (saving / base_cost * 100.0) if base_cost else 0.0
        print(
            f"{city_name} HVAC cost saving: "
            f"{saving:.2f} USD ({saving_pct:.2f}%)"
        )
