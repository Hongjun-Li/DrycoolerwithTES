...
# 4. 绘图：仅保留水路温度，左右双子图，共享纵轴
fig, axes = plt.subplots(1, 2, figsize=(12, 4), sharey=True)
plt.subplots_adjust(wspace=0.15)

t_ctrl_cdu_sup = np.array(res_ctrl['yTCDUSup']) - 273.15
t_ctrl_cdu_ret = np.array(res_ctrl['yTCDURet']) - 273.15
t_base_cdu_sup = np.array(res_base['yTCDUSup']) - 273.15
t_base_cdu_ret = np.array(res_base['yTCDURet']) - 273.15

axes[0].plot(res_ctrl['time'], t_ctrl_cdu_sup, color='blue', linewidth=1.5, label='T_CDU_Sup')
axes[0].plot(res_ctrl['time'], t_ctrl_cdu_ret, color='purple', linewidth=1.5, label='T_CDU_Ret')
axes[0].plot(res_base['time'], t_base_cdu_sup, color='blue', linestyle='--', alpha=0.6, label='T_CDU_Sup (Base)')
axes[0].plot(res_base['time'], t_base_cdu_ret, color='purple', linestyle='--', alpha=0.6, label='T_CDU_Ret (Base)')
axes[0].set_ylabel('Temp [°C]')
axes[0].set_title('CDU Temperatures')
axes[0].legend(loc='upper right', fontsize='small', ncol=1)

t_ctrl_cw_lea = np.array(res_ctrl['yTCWLeaTow']) - 273.15
t_ctrl_cw_ent = np.array(res_ctrl['yTCWEntTow']) - 273.15
t_base_cw_lea = np.array(res_base['yTCWLeaTow']) - 273.15
t_base_cw_ent = np.array(res_base['yTCWEntTow']) - 273.15

axes[1].plot(res_ctrl['time'], t_ctrl_cw_lea, color='orange', linewidth=1.5, label='T_CW_Lea_Tow')
axes[1].plot(res_ctrl['time'], t_ctrl_cw_ent, color='red', linewidth=1.5, label='T_CW_Ent_Tow')
axes[1].plot(res_base['time'], t_base_cw_lea, color='orange', linestyle='--', alpha=0.6, label='T_CW_Lea_Tow (Base)')
axes[1].plot(res_base['time'], t_base_cw_ent, color='red', linestyle='--', alpha=0.6, label='T_CW_Ent_Tow (Base)')
axes[1].set_title('CW Temperatures')
axes[1].legend(loc='upper right', fontsize='small', ncol=1)
